...
class MyFrame():
    ...
    def forward(self, volatile=False):
        self.img = V(self.img.cuda(), volatile=volatile,requires_grad=True)
        if self.mask is not None:
            self.mask = V(self.mask.cuda(), volatile=volatile,requires_grad=True)
    ...
    def optimize(self):
        ...
        self.mask = torch.squeeze(self.mask,1)
        loss = self.loss(pred, self.mask.long())
        loss.backward()
        self.optimizer.step()
        return loss.item()
